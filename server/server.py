...
def run():                   #Main loop
    ...
    while True: 
        ...
        if not data:
            ...
            try:
                angle=float((str(data))[8:].strip())      #Steering
            except:
                pass
                '''
                angle = 0
            ...
            global spd_ad
            try:
                spd_ad=float((str(data))[6:].strip())      #Speed Adjustment
            except:
                motor.motorStop()
            ##print(str(data))
            #print(spd_ad)        
            if spd_ad < 0: # Forward
                ...
                motor.motor_left(status, forward,left_spd1*(-spd_ad))
                motor.motor_right(status,backward,right_spd1*(-spd_ad))
                colorWipe(strip, Color(0,0,255))
            elif spd_ad > 0: # backward
                tcpCliSock.send('2'.encode())
                motor.motor_left(status, backward, left_spd1*spd_ad)
                motor.motor_right(status, forward, right_spd1*spd_ad)
                colorWipe(strip, Color(255,0,0))
            else:
                motor.motorStop()
